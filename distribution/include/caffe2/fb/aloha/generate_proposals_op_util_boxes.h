...
EArrXXt<typename Derived1::Scalar> bbox_transform(
  ...
  auto dx = deltas.col(0).template cast<T>();
  auto dy = deltas.col(1).template cast<T>();
  auto dw = deltas.col(2).template cast<T>().cwiseMin(bbox_xform_clip);
  auto dh = deltas.col(3).template cast<T>().cwiseMin(bbox_xform_clip);
